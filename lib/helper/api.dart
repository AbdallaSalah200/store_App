...
Future<dynamic>get({required String uri,@required String ? token })async{
  ...
  if(response.statusCode==200)
  {
    ...
    throw Exception('there is apromplem in ${response.statusCode}');
  }
}
Future<dynamic>post ({required String url,@required dynamic body,@required String ? token}) async
{
  ...
  http.Response response= await    http.post(Uri.parse(url),body: 
       ...
       return data ;
        ...
        throw Exception('there is aproblem with status codr ${response.statusCode},with body ${response.body}');
       }
}
